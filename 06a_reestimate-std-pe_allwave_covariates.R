#---------------------------------------------------#
# Re-estimate PE models for variables assessed at   #
# all waves with additional covariates.             #
#---------------------------------------------------#
...
for (outcome in outcome_varList) {
  ...
  fmla <- as.formula(paste(outcome, "~ AGE + NAS201TRAN  + TEDALL + CHARLSON + Race + Ethnicity + WAVE2 + WAVE3 + WAVE4 + WAVE2_ASSESSMENT2 + WAVE3_ASSESSMENT2 + WAVE3_ASSESSMENT3 + WAVE4_ASSESSMENT3 + WAVE4_ASSESSMENT4 + SKIP1 + SKIP2"))
...
write.csv(wide_results, paste0("results/gee_standardized_results_allwave_", date, "_covariates.csv"), row.names = FALSE)
