#--------------------------------------------------------------------------------#
# Re-estimate PE models for Boston naming test span with additional covariates.  #
#--------------------------------------------------------------------------------#
...
for (outcome in outcome_varList) {
  ...
  gee_data <- gee_data %>%
    mutate(
      ...
      TEDALL = (TEDALL - wave3_stats$edu_mean) / wave3_stats$edu_sd,  # Wave 3 standardization
      ...
      !!outcome := (!!sym(outcome) - wave3_stats$outcome_mean) / wave3_stats$outcome_sd  # Wave 1 standardization
    )
  
  # Create formula for GEE model.  
  # Note: We can use caret::findLinearCombos(model_matrix) to verify no linear combinations
  fmla <- as.formula(paste(outcome, "~ AGE + NAS201TRAN  + TEDALL + CHARLSON + Race + Ethnicity + WAVE4"))
...
write.csv(wide_results, paste0("results/gee_standardized_results_bnt_", date, "_covariates.csv"), row.names = FALSE)