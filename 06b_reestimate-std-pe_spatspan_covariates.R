#---------------------------------------------------------------------#
# Re-estimate PE models for spatial span with additional covariates.  #
#---------------------------------------------------------------------#
...
# Create race and ethnicity variables
admin = admin %>% 
  mutate(
    Race = case_when(
...
admin <- admin %>% select(VETSAID=vetsaid, starts_with("AGE"), Race, Ethnicity, TEDALL)
...
# Merge Charlson with admin data
admin <- admin %>% left_join(charlson, by=c("VETSAID"))
...
# Pivot admin file to long format
admin_long <- admin %>% 
  pivot_longer(cols = -c(VETSAID, Race, Ethnicity, TEDALL),  
               names_to = c(".value", "WAVE"),
               names_pattern = "(\\w+)_V(\\d+)",
               values_drop_na = TRUE) %>%
...
for (outcome in outcome_varList) {
  ...
  # Here, we add education and Charlson index as a sensitivity analysis.
  geeCols = c("AGE", "NAS201TRAN", "TEDALL", "CHARLSON", "Race", "Ethnicity",
              "WAVE2", "WAVE3", "WAVE2_ASSESSMENT2", "WAVE3_ASSESSMENT2", "WAVE3_ASSESSMENT3",
              "SKIP1")
  ...
  # Calculate Wave 1 means and SDs for outcome, NAS201TRAN and TEDALL
  wave1_stats <- gee_data %>%
    filter(WAVE == 1) %>%
    summarise(
      nas_mean = mean(NAS201TRAN, na.rm = TRUE),
      nas_sd = sd(NAS201TRAN, na.rm = TRUE),
      edu_mean = mean(TEDALL, na.rm = TRUE),
      edu_sd = sd(TEDALL, na.rm = TRUE),
      outcome_mean = mean(!!sym(outcome), na.rm = TRUE),
      outcome_sd = sd(!!sym(outcome), na.rm = TRUE)
    )
  
  # Calculate full sample statistics for age and charlson
  age_stats <- gee_data %>%
    summarise(
      age_mean = mean(AGE, na.rm = TRUE),
      age_sd = sd(AGE, na.rm = TRUE),
      charlson_mean = mean(CHARLSON, na.rm = TRUE),
      charlson_sd = sd(CHARLSON, na.rm = TRUE)
    )
  ...
  gee_data <- gee_data %>%
    mutate(
      ...
      TEDALL = (TEDALL - wave1_stats$edu_mean) / wave1_stats$edu_sd,  # Wave 1 standardization
      CHARLSON = (CHARLSON - age_stats$charlson_mean) / age_stats$charlson_sd,  # full sample standardization
  ...
  fmla <- as.formula(paste(outcome, "~ AGE + NAS201TRAN  + TEDALL + CHARLSON + Race + Ethnicity + WAVE2 + WAVE3 + WAVE2_ASSESSMENT2 + WAVE3_ASSESSMENT2 + WAVE3_ASSESSMENT3 + SKIP1"))
...
write.csv(wide_results, paste0("results/gee_standardized_results_spatspan_", date, "_covariates.csv"), row.names = FALSE)